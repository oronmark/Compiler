	...
	(lambda (pe c-table env)
		(let* ((var (get-set-var pe))
		  ...
		  (string-append
		  	...
		  	"MOV(INDD(R0," (number->string minor) "), R0);\n"
		  	"MOV(R0,IND(R0));\n\n"))))
...
; 	'(append apply < = > +  * - char->integer   denominator  /
...
(define runtime-support-functions
	'(car cdr integer? char? pair? procedure? boolean? rational? null? string? symbol? string->symbol symbol->string binary-div cons))
...
(define my-binary-div
	(lambda ()
		(let ((address (get-fvar-address '(fvar binary-div) global-fvar-table)))
		(string-append
            "//BINARY-DIV\n"
